...
public partial class ProfilesView
{
    ...
    public ProfilesView()
    {
        ...
        // 订阅语言切换事件
        Manager.LanguageManager.LanguageChanged += OnLanguageChanged;
    }
    ...
    private void OnLanguageChanged(string languageCode)
    {
        ...
        Application.Current?.Dispatcher.Invoke(() =>
        {
            try
            {
                ...
                {
                    if (column is MyDGTextColumn myColumn)
                    {
                        // 根据列名获取对应的资源键
                        var resourceKey = myColumn.ExName switch
                        {
                            "type" => "String_LvServiceType",
                            "remarks" => "String_LvRemarks",
                            "address" => "String_LvAddress",
                            "port" => "String_LvPort",
                            "streamSecurity" => "String_LvTLS",
                            "network" => "String_LvTransportProtocol",
                            "subid" => "String_LvSubscription",
                            "testResult" => "String_LvTestDelay",
                            "speed" => "String_LvTestSpeed",
                            _ => null
                        };
                        ...
                        if (resourceKey != null)
                        {
                            ...
                            if (resource is string headerText)
                            {
                                myColumn.Header = headerText;
                            }
                        }
                    }
                }
            ...
            {
                Logging.SaveLog("ProfilesView.OnLanguageChanged", ex);
            }
        });
    }
